...
import { registerCustomer, getCustomer, updateCustomer, deleteCustomer } from "../../api/CustomerApi.js";
import './Customer.css'
import { useNavigate } from "react-router-dom";
import { getCompany } from "../../api/CompanyAPI.js";
import { getAreaByCompany } from "../../api/CompanyAPI.js";
...
function Customer() {
    ...
    async function fetchCustomer() {
        try {
            const data = await getCustomer(riderId);
            setRecords(data);
        } catch (error) {
                ...
                {records.length > 0 ? (
                        records.map((customer) => (
                            <tr key={customer.customer_id}>
                                <td onClick={() => navigate(`/dashboard/monthly/${customer.customer_id}`)}>{customer.account_no}</td>
                                <td>{customer.name}</td>
